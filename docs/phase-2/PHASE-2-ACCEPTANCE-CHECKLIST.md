...
**Status:** Open; all four P1 findings and all eight P2 findings are fixed, locally verified, and committed as of 14 September 2026 (`36223ae`, then `aede3a5`, both pushed; hosted CI passed on `aede3a5`), while external acceptance and the PR #1 merge decision remain open
...
Do not merge or describe Phase 2 as accepted while the external acceptance gates remain open. The P2 snapshot has been independently reviewed (PR 2), committed, and passed hosted CI; that closes the repository gates, not the external ones.
...
- [x] [Review finding 4](./PR-1-REVIEW-2026-09-13.md#4-an-earlier-applied-revision-of-the-phase-2-migration-is-treated-as-current): the forward-only [`20260913230000_reconcile_phase2_scanner_migration_drift`](../../backend/prisma/migrations/20260913230000_reconcile_phase2_scanner_migration_drift/migration.sql), SHA-256 `d2f4805aef790c484fd1878edecfcba0a39b93529cb8d9b0879469e2aaa99d46`, repairs only a legacy catalog that matches its precondition inventory, which was reconstructed in [`phase2-legacy-scanner.sql`](../../backend/tests/fixtures/phase2-legacy-scanner.sql) because the `a0e4f792...` migration text itself is not in the repository. [`migrationGuard.ts`](../../backend/src/config/migrationGuard.ts) fails closed on unsupported checksums, altered CHECK/FK/index semantics, missing migration assets, and reachable ledger or catalog inspection failures. [`verify-phase2-reconciliation.ts`](../../backend/scripts/migrate-workflow/verify-phase2-reconciliation.ts) executes the reconstructed legacy upgrade and seven malformed-state rejections (including the unknown-checksum and both `partially present` branches since rounds 4 and 5) against disposable PostgreSQL (see the P1 local verification entry below for exactly which branches it reaches).
...
The P1 and P2 items above record implemented and locally verified fixes. Nothing has been waived. The external acceptance gates further down remain open.
...
- [x] The supported-legacy reconciliation fixture and the fresh chain both produced no Prisma schema difference and the same 235-fact canonical Phase 2 catalog and security digest. The executable legacy harness (committed in `36223ae`) also reports a clean startup guard. The harness builds nine disposable databases (six until round 4, eight until round 5) and proves exactly this: the fresh chain and the reconstructed legacy fixture reach an identical catalog with no Prisma schema difference and a clean startup guard; a legacy database whose `ReceiptScan_batch_link_check` was weakened is rejected with no partial repair objects left behind; and a current-checksum database is rejected when the `ReceiptPurgeJob_active_profile_receipt_key` predicate is altered, when `ReceiptScan_profile_source_image_hash_idx` uses the `bpchar_pattern_ops` operator class, or when that index uses `COLLATE "C"`. Since 14 September (round 4) it also builds two more: a legacy-shaped database whose `20260913100918` ledger row carries a checksum that is neither `8e03ef8f...` nor `a0e4f792...` is rejected by `prisma migrate deploy` with `Unsupported checksum for 20260913100918_receipt_capture_batches_and_scan_revision: <checksum>.`, and a legacy-shaped database where the `ReceiptPurgeMode` enum already exists is rejected with `Unsupported legacy Phase 2 shape: later Phase 2 objects are partially present.`; in both cases the harness asserts that no repair object exists afterwards and that the ledger holds no completed, non-rolled-back row for the reconciliation migration (Prisma leaves an unfinished row). Since 14 September (round 5) it also builds a ninth: a legacy-shaped database where the replacement index `ReceiptScan_history_created_idx` already exists (created with the migration's own definition) is rejected by `prisma migrate deploy` with `Unsupported legacy Phase 2 shape: replacement indexes or constraints are partially present.`; the harness asserts that no repair object exists afterwards, that the pre-created index is the only replacement index or constraint present, that the two legacy indexes the repair path drops (`ReceiptScan_BusinessProfile_ID_idx`, `ReceiptPurgeJob_profile_receipt_key`) survive, and that the ledger holds no completed, non-rolled-back row for the reconciliation migration. Seven rejections are exercised in all. The legacy fixture [`phase2-legacy-scanner.sql`](../../backend/tests/fixtures/phase2-legacy-scanner.sql) (SHA-256 `f63fe18dd045922f33287d4d74663fa7c79555778143c3b9807326c6fcc9b448`) is a reconstruction of the `a0e4f792...` revision's catalog; no git object holds a migration file with that checksum, so the harness proves the migration accepts the reconstructed shape, not the recorded revision itself.
...
All commands ran under Node 22.23.2 with `DATABASE_URL` and `DIRECT_URL` pinned to disposable local PostgreSQL 16 databases. The snapshot was HEAD `28394be` plus the then-uncommitted worktree, since committed as `36223ae`; complete backend, mobile, and web unit suites ran on tracked-diff SHA-256 `9d9a81e4f0c8e9080ad630e9ba5cd283839e7a52715c13fc2aa40850ff2b07a1`, after which the only change was a one-line e2e selector in `web/e2e/phase2-receipt-review.spec.ts`, and the final Chromium run used tracked-diff `bf9248724f7e495fc2eed4096443927ed6524d6128df6d2fbf5db24850cb8cc3`. These tracked-diff fingerprints identified the worktree at the time and cannot be recomputed now that the work is committed; the post-commit evidence is the hosted CI run on `aede3a5`.

- [x] Stage 0A closed first: the exact P1 snapshot (tracked-diff `08d93d04cba4652d04a5af4f7285c8d16dafc038cc5a5c6c5a0bbf43474ba7ed` at the time; not recomputable after the commit) passed the complete backend suite on a fresh database with the frozen reconciliation migration unchanged: 151 files, 2,245 passed, 1 skipped, 2,246 total, 44 migrations, status current, diff clean, guard `ok`, zero remaining sessions.
...
- [x] Reconciliation harness: fresh and reconstructed-legacy paths share 237 facts at digest `eb4d357dd91fe66da9f52557dc500b9b6859ed5da9a8f48deedbea886d9c8c50`; the weakened legacy CHECK, altered predicate, collation, and operator-class states were rejected; harness containers removed.
...
After the PR 2 fixes, on the snapshot with tracked-diff SHA-256 `c839590ac02a6f841d9882402460b958fdaf9429ffa561665f7a4e3549f02c01` (HEAD `28394be` plus the then-uncommitted worktree; the fingerprint is not recomputable after the commit):
...
- [x] Reconciliation harness with the extended guard: 237 facts at `eb4d357dd91fe66da9f52557dc500b9b6859ed5da9a8f48deedbea886d9c8c50`; the same four malformed states rejected. Re-run on 14 September after the harness mounts were narrowed to the migrations directory and the fixture (no longer the whole `backend/` tree): 237 facts at the same digest, on a worktree with 47 migrations: the 45 at `aede3a5` plus `20260914104500_external_processing_consent_source` and `20260914104600_receipt_scan_last_activity_grace`, added by the database owner on 14 September.
...
- [x] Committed and pushed. The fixes were committed on 14 September 2026 as `36223ae` (P1 and P2 fixes, reconciliation migration, `20260914010610`, harness, legacy fixture, and these documents) and `aede3a5` (provider-first routing and policy consent), both pushed to `finsightv1/feat/phase2-scanner-acceptance`. The hosted CI push run on `aede3a5` passed (run 34799460479). PR #1 against `main` is open; its pull-request run was still in progress when this was audited. The gates above ran on the pre-commit worktree; hosted CI on `aede3a5` is the post-commit evidence.
- [x] The pull-request run on `aede3a5` also completed successfully (run 34799462649).
- [x] Round 3 (P3 backlog and follow-ups, worktree on top of `aede3a5`): backend 163 files, 2,396 passed, 1 skipped on a fresh 48-migration database with guard `ok`; web 86 / 770 plus 18 end-to-end; mobile 98 / 1,090; parity, policy-v1, harness (237 facts) green. See the PR 2 review's round 3 section.
- [x] Round 3 committed as `c3e4484` and pushed; hosted CI passed on it (push run 34803733413, pull-request run 34803736058).
- [x] Round 4 (type parity, post-commit isolation, replay gates, guard grant sentinels, harness six rejections): backend 163 / 2,401 / 1 skipped; web 86 / 772 + 18 e2e; mobile 98 / 1,092; parity clean. See the PR 2 review's round 4 section.
- [x] Round 4 committed as `ef6dcf5` and pushed; hosted CI passed on it (push and pull-request runs).
- [x] Round 5 (update-path isolation, ninth harness scenario, shape alignment): backend 163 / 2,402 / 1 skipped; harness seven rejections; web 86 / 772 + 18 e2e; mobile 98 / 1,092; parity clean. Repository backlog from the reviews is empty.
- [x] Round 5 committed as `b218ec0` and pushed; hosted CI passed on it (push and pull-request runs). PR #1 title and description updated to cover all six commits.
- [x] PR #1 merged by the owner as `ef8ea68` on `main` at 05:45 UTC, 14 September 2026.
- [x] Owner-reported device pass, Poco X6 5G, 14 September 2026 (post-round-5 code on the development API; the installed APK is still the 12 September build, so the JS-side changes were exercised through the API and review screens, not a rebuilt APK): rescanning the crumpled receipt showed Gemini's line items flagged for review ("yes"); a paper receipt on a plain surface was captured by the native `Capture now` ("yes"). The logout cache check and the Android version were not reported. The earlier native-capture P1 therefore reduces to a scene-dependence finding: the detector refuses busy or low-contrast backgrounds, as its guidance says, and works on a plain surface. Standard capture on a full device matrix, Long capture, permissions, lifecycle, thermal, and TalkBack remain untested.
- [x] Authorized read-only hosted verification run by the owner (14 September, about 05:50 UTC): 48 finished migrations with every Phase 2 checksum matching the files on disk (including the frozen reconciliation migration), guard `ok`, 35 tables with RLS on and zero policies, no `anon`/`authenticated`/`PUBLIC` table grants. Storage policies, backups, and purge drills remain open.
- [ ] Hosted deployment and authorized hosted verification remain open. Three accidental SELECT-only verification entrypoints reached the hosted database during follow-up, made no write or migration, accessed no user or receipt data, and do not count as an acceptance pass. The owner personally deployed `20260913192200` and `20260913194745` on 13 September and `20260913230000` and `20260914010610` at about 02:07 on 14 September from the interactive shell to keep the development server running; the hosted ledger is expected to hold all 45 migrations (the 45 committed at `aede3a5`; the two later worktree migrations `20260914104500_external_processing_consent_source` and `20260914104600_receipt_scan_last_activity_grace`, which bring the chain to 47, are not part of that expectation and are not recorded as deployed anywhere), unverified by an authorized check. See [HOSTED-MIGRATION-INCIDENT.md](./HOSTED-MIGRATION-INCIDENT.md).
- [ ] Physical-device camera acceptance remains open. Local render, unit, integration, and Android automation cannot close it. Owner-reported on 14 September 2026 at about 02:10 PHT: the standalone Android APK on a Poco X6 5G (Android version not yet recorded) reached the development API over Tailscale and signed in successfully after the owner deployed the two outstanding migrations. That is connectivity and auth evidence only. Later the same morning the owner tried the native Standard capture on the same device (pre-P2 APK build): tapping `Capture now` changed the guidance to "Checking receipt edges…" and the detector never accepted a frame, with no missing-edge or quality message, for as long as the owner kept trying. The first attempt was a receipt image on a screen (not a valid scene); the owner then retried with a real paper receipt and got the same result: "Checking receipt edges…" indefinitely, no capture, no missing-edge or quality message. The scene was a wrinkled receipt on patterned pink fabric, which the guidance says to avoid; a flat receipt on a plain contrasting surface is the pending retest that decides the finding. Manual capture of the same scene produced a confidence-44 read that the app correctly flagged for review (total extracted, vendor wrong, no date or items). The owner then enabled the Gemini rescue provider on the development project (owner decision, 14 September, about 09:30 PHT: `RECEIPT_PROVIDER=gemini`, dispatch enabled, kill switch off, data terms and routing calibration marked approved for development, 100-unit monthly cap, business consent accepted in the app). A new scan of the same receipt dispatched to Gemini (`PROVIDER_OK`) and the review screen showed the line items. The owner then switched the development project to `RECEIPT_PROVIDER_ROUTING=always` and `RECEIPT_PROVIDER_CONSENT_MODE=automatic` (opt-in settings added the same day; defaults unchanged; see the PR 2 review, device finding 5). One earlier scan crashed after a successful dispatch and its retry correctly refused a second paid call; the cause predates the new worker diagnostics. Android version not yet recorded. See the PR 2 review's device findings. Manual sections and Gallery remain available on the same screen and were suggested as the path for the rest of the device journey.
...
The four P1 and eight P2 findings are resolved, locally verified, and committed, and the PR 2 dispositions are recorded. Phase 2 remains open until physical-device evidence passes, deployed private-storage and purge checks pass, and the consented corpus report contains no failed mandatory gate. Any unavailable evidence remains named. A capstone pass must still state that it is not a production accuracy claim.
