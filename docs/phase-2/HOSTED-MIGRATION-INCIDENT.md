...
Local disposable-database tests pinned both `DATABASE_URL` and `DIRECT_URL`. Fresh and reconciled legacy paths produced the same 235-fact canonical Phase 2 catalog and security digest, `5b9fbff1a7f8d9aeb3ca15ebdf484e5078d81ec4541cde9fc716ced4a45e39d3` on the P1 snapshot, and an identical 237-fact catalog at `eb4d357dd91fe66da9f52557dc500b9b6859ed5da9a8f48deedbea886d9c8c50` after the 14 September `20260914010610_receipt_scan_last_activity` migration, The harness that produced those digests, `backend/scripts/migrate-workflow/verify-phase2-reconciliation.ts`, exercises seven rejections and no more: a weakened legacy `ReceiptScan_batch_link_check` (with no partial repair left behind), an altered `ReceiptPurgeJob_active_profile_receipt_key` predicate, a `bpchar_pattern_ops` operator class, a `COLLATE "C"` collation on `ReceiptScan_profile_source_image_hash_idx`, and, since 14 September (round 4), a `20260913100918` ledger row with an unrecognized checksum (rejected by `prisma migrate deploy` with the migration's `Unsupported checksum` text) and a legacy database where the `ReceiptPurgeMode` enum already exists (rejected with `later Phase 2 objects are partially present`); both of those assert no repair object was left behind and no completed reconciliation ledger row exists; and, since round 5 later on 14 September, a legacy database where the replacement index `ReceiptScan_history_created_idx` already exists (rejected with `replacement indexes or constraints are partially present`, asserting the same absence of repair objects and completed ledger row, that the pre-created index is the only replacement object present, and that the two legacy indexes the repair path drops survive). The harness reproduced the unchanged 237-fact catalog at `eb4d357d...` with the two round-4 scenarios and again with the round-5 scenario on 14 September. Its legacy fixture, `backend/tests/fixtures/phase2-legacy-scanner.sql`, is a reconstruction of the `a0e4f792...` catalog, not the recorded migration text, which exists in no git object. The reconciliation migration was deployed to the hosted development project by the owner at about 02:07 PHT on 14 September (see the implementation evidence), not by an agent.
...
## Authorized read-only hosted verification, 14 September 2026

At about 05:50 UTC on 14 September 2026 the owner ran a read-only check against the development project (host `aws-0-ap-northeast-1.pooler.supabase.com`, via the repository's configured connection) and shared the output. It reported: ledger 49 rows, 48 finished, 0 unfinished (one rolled-back row, the normal Prisma record of a failed-then-resolved migration); every Phase 2 migration present with a checksum matching the file on disk, including `20260913100918` at `8e03ef8f…` (the current source revision, not the drifted `a0e4f792…`), `20260913230000` at `d2f4805a…`, and the three round-3 migrations; the live schema-shape guard `{"status":"ok"}` with no pending, failed, checksum-mismatch, or schema issues; 35 application tables all with row-level security enabled and zero policies; and no direct table grants to `anon`, `authenticated`, or `PUBLIC`. This closes the hosted ledger and schema-shape gate for the development project. It does not cover Storage bucket policies, backups, or the purge drills, which remain open.

Nothing in the sections above authorizes a further hosted operation. Both earlier deploys were the owner's own interactive actions on the development project. Before the verification the hosted ledger was expected to hold all 45 migrations (the 45 committed at `aede3a5`; the two later worktree migrations `20260914104500_external_processing_consent_source` and `20260914104600_receipt_scan_last_activity_grace`, which bring the chain to 47, are not part of that expectation and are not recorded as deployed anywhere), but that expectation rests on the owner's report and the guard's pass, not on an authorized read-only ledger and schema-shape check, which is still required before the hosted state can be called verified.
