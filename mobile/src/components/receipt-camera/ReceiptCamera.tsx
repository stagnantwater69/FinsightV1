  ...
  return <View style={[styles.root, { backgroundColor: t.cameraSurface, paddingTop: insets.top, paddingBottom: Math.max(insets.bottom, 12) }]}>
    ...
    {cropping && selected ? <CropEditor key={selected.localId} uri={selected.originalUri} width={selected.originalWidth ?? selected.width} height={selected.originalHeight ?? selected.height} initial={selected.cropCorners} busy={busy} onApply={corners => void applyCrop(corners)} onCancel={() => setCropping(false)} onDetect={detect} /> : <>
      <View style={styles.viewfinder}>
        {/* `accessible` on both: React Native leaves an Image out of the accessibility tree unless it is given `alt` or told to be an element, so these labels were written and never read. */}
        {selected ? <Image accessible accessibilityRole="image" accessibilityLabel={`Preview of receipt section ${sections.indexOf(selected) + 1}`} source={{ uri: selected.processedUri }} style={StyleSheet.absoluteFill} resizeMode="contain" /> : readyToReview ? <Image accessible accessibilityRole="image" accessibilityLabel="Receipt awaiting review" source={{ uri: sections[0]!.processedUri }} style={StyleSheet.absoluteFill} resizeMode="contain" /> : showCamera ? <>
