/**
 * Help and legal copy for the app — a MIRROR of web/src/lib/marketingContent.ts.
 *
 * WHY A COPY AND NOT AN IMPORT: web and mobile are separate packages with no
 * workspace linking them, and Metro will not bundle a file from outside the
 * mobile project root. The same constraint already forced receiptConfirm.ts to
 * exist twice, and the same answer applies — copy it, then make the copy
 * unable to drift.
 *
 * WHAT KEEPS THEM IDENTICAL: tests/helpContent.test.ts imports BOTH this file
 * and the web original across the project boundary and asserts they match,
 * exactly as backend/tests/contract does for the two receipt payload builders.
 * Editing one without the other fails that test.
 *
 * WHY IT MATTERS MORE HERE THAN FOR MOST DUPLICATION: the panel found most
 * small business owners have a phone and no computer, so for a great many of
 * them the app IS where they will read what FinSight does with their records.
 * A privacy notice that says two different things depending on which screen
 * you read it on is worse than one that says nothing.
 *
 * Anything genuinely mobile-only belongs in a different file. This one is a
 * mirror, and a mirror with an extra line in it is no longer a mirror.
 */
