/**
 * What window Home's figures cover, and how that window is named.
 *
...
/** Home's default window: the last 30 days, ending today. */
export const DEFAULT_SUMMARY_PERIOD_DAYS = 30;

/**
 * The windows Home offers, in the order they sit in the segmented control.
 *
 * Labelled by their length rather than by a calendar name for the reason
 * above, and kept short because four of them share the width of a 360dp
 * screen. "All time" exists because CSV import invites owners to bring in
 * years of history: without it, a business whose records all predate the last
 * 30 days has no setting on this page that can see a single one of them.
 */
