 ...
 *
 * Mirrored by web/src/lib/authValidation.ts, deliberately rather than shared —
 * the two apps have no build-time relationship, and the note at the top of
 * theme/tokens.ts explains why a package to share this would couple their
 * release cycles for very little. The tests on both sides assert the same
 * figures, which is what keeps the copies honest.
 *
 * Nothing here imports React or react-native, so it runs under plain vitest.
 */
