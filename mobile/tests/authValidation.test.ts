 ...
 *
 * web/src/lib/authValidation.test.ts asserts the same figures against its own
 * copy, which is what keeps the two clients honest with each other.
 */
...
/**
 * The emailed recovery code, which is the ONLY way a reset is finished now.
 *
 * The reset email's button is gone from the Supabase template, so nothing
 * opens the app from that inbox any more and these rules are the whole of what
 * the phone can check before asking Supabase. web/src/lib/authValidation.test.ts
 * asserts the same figures against web's copy — the two forms have to accept
 * and refuse exactly the same codes, or the same email works on one client and
 * not the other.
 */
describe("validateRecoveryCode", () => {
   ...
   * and "7532-4744" are what people type back. Supabase compares the token as a
   * string, so without this either of them is a wrong code with no visible
   * cause. Only spaces and dashes are forgiven — anything else is a character
   * the owner meant, and deleting it silently would hide a real typo.
   */
  it("forgives the spaces and dashes people paste, and nothing else", () => {
    expect(normaliseRecoveryCode(" 7532 4744 ")).toBe("75324744");
    expect(normaliseRecoveryCode("7532-4744")).toBe("75324744");
    expect(isValid(validateRecoveryCode({ email, code: "7532 4744" }))).toBe(true);
    expect(isValid(validateRecoveryCode({ email, code: "7532-4744" }))).toBe(true);
    expect(validateRecoveryCode({ email, code: "7532.4744" }).code).toBeTruthy();
  });
