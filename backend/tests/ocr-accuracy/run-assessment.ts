...
(async () => {
  ...
  lines.push("");
  lines.push("5. **A footer vendor name lost to a garbled document-header banner (defect, fixed).** See failure pattern 2 above — decorative banner punctuation and closing boilerplate ('thank you, come again') are now excluded from vendor candidates regardless of OCR spelling noise, and the Filipino 'tindahan' was added alongside 'store' as a recognised business-type word.");
  lines.push("");
  lines.push("Measured effect across all five: date accuracy 90% → **100%**, vendor accuracy 89% → **100%** on this corpus, all-three-correct 16/20 → **30/31**. Every fix is pinned by regression tests in `tests/unit/ocrParsing.test.ts`.");
  lines.push("");
