...
export function SafeguardsGrid() {
  return (
    <section
      ...
      className="landing-privacy-surface relative overflow-hidden"
    >
            ...
            {SAFEGUARDS.map((s, i) => {
              ...
              return (
                <Rise key={s.title} delay={i * 70} className="h-full">
                  <div className="h-full rounded-2xl border border-landing-mint-light bg-landing-surface p-6 shadow-sm">
                    ...
                    <span className="flex h-10 w-10 items-center justify-center rounded-xl border border-landing-mint-light bg-landing-mint-pale">
                      <Icon className="h-5 w-5 text-landing-green" aria-hidden />
                    </span>
                    <h3 className="mt-4 font-landing-display text-base font-bold leading-snug text-landing-charcoal">{s.title}</h3>
                    <p className="mt-2 text-sm leading-relaxed text-landing-muted">{s.body}</p>
                  </div>
                </Rise>
              );
            })}
