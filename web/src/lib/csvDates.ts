 ...
 * guesses month-first, parses in the BROWSER's local time (UTC+8 in this
 ...
 *
 * This file closes the drift the mobile client reported: web's row check
 * used to run `new Date(rawDate)` and would therefore mark a row valid that
 * the server then skipped (and vice versa) — a preview that lies about which
 * rows need fixing. Web, mobile and the server now answer identically, which
 * is what mobile/tests/webParity.test.ts is for.
 *
...
// The type itself lives in lib/types.ts with the rest of the API contract —
// re-declaring it here would let the two drift apart silently.
