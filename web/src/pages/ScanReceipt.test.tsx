...
vi.mock("../context/ExpenseCategoryContext", () => ({ useExpenseCategories: () => ({
  categories: [{ id: 2, name: "Supplies" }], refresh: async () => {}, createCategory: vi.fn(), loading: false,
}) }));
...
beforeEach(() => {
  ...
  mocks.post.mockImplementation(async (url: string) => ({ data: url === "/ai/suggest-category" ? { suggestion: null } : receipt }));
});
...
describe("receipt upload and review", () => {
  ...
  it("resumes an accepted scan after a failed poll without uploading again", async () => {
    ...
    mocks.get.mockRejectedValueOnce(new Error("Network interrupted")).mockResolvedValue({ data: receipt });
    render(page());
    await user.upload(screen.getByLabelText(/Receipt photo/), photo());
    await user.click(screen.getByRole("button", { name: "Scan receipt" }));
